...
class iSql {
	...
	function iSql_sql($value = array(),$return = false,$sw = true) {
		if(empty($value)){
    		return false;
		}
		$u = count($value);
	...
	function iSql_add($db, $data = array()){
	...
	function iSql_del($db, $data = array()){
	...
	function iSql_search($db, $data = array()){
		if(empty($data)){
			$val = "SELECT * FROM $db";
	...
	function iSql_edit($db,$set = array(),$data = array()){
