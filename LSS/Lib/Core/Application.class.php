...
final class Application {
    public static function run () {
        ...
        self::_app_run();
        //输出控制台信息
        if(empty($_GET['console'])) $_GET['console'] = 'yes';
        if(CONSOLE_ON && $_GET['console'] != 'no'){
            $etime = microtime(true);//获取程序执行结束的时间  
            $total = round($etime - TIMER,3);   //计算差值
            ...
            $mb = $temp/1000;
            $mb = round($mb/1024,2);
            $all_log = $GLOBALS['DB_LOG']['sql'] + $GLOBALS['DB_LOG']['read'] + $GLOBALS['DB_LOG']['write'] + $GLOBALS['DB_LOG']['delete'];
            require SYS_TPL_PATH . '/console.html';
        }
		clearstatcache();
    }
...
str;
    ...
    private static function _set_url () {
        ...
        define('__TOOL__', __ROOT__ . '/'.C('CORE_TPL').'/Extends/Tool');
        define('__ORG__', __ROOT__. '/'.C('CORE_TPL').'/Extends/Org');
        define('__CORE__', __ROOT__. '/'.C('CORE_TPL').'/');
    ...
    private static function _app_run () {
        ...
        if(class_exists($c)){
            ...
            if(!method_exists($obj, $a)){
                ...
                if(!empty($par)){
                    call_user_func_array(array($obj, $a), $par);
                }else{
                    $obj -> $a();
                }
        ...
        switch($errno){
            ...
            default:
                if(DEBUG) {
                    ...
                    //$url = __CORE__ . 'Data/Tpl/notice.php';
                    //echo "<script>window.open('$url','','width=500,height=300' )</script>";
                }
                break;
        }
    }
